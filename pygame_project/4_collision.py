...
character_y_pos = screen_height - character_hight - stage_hight
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False  
        if event.type == pygame.KEYDOWN: # 캐릭터를 왼쪽으로
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                ball_to_romove = ball_idx #해당 공 없애기 위한 값 설정
                break
    #충돌된 공 or 무기 없애기
    if ball_to_romove > -1:
        del balls[ball_to_romove]
        ball_to_romove = -1
